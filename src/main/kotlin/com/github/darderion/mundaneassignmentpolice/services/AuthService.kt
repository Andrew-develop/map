...
class AuthService (
    ...
    fun createNewUser(@RequestBody request: UserDto, sub: Subscription, rulePacks: List<RulePack>): ResponseEntity<*> {
        if (userService.findForRegistration(request.email) != null) {
            return ResponseEntity(AppError(HttpStatus.BAD_REQUEST.value(), "Пользователь с указанным email уже существует"), HttpStatus.BAD_REQUEST)
        }
        val user = userService.createNewUser(request, sub, rulePacks)
